...
string Pais::getCodigoISO() { return codigoISO; }

string Pais::getNome() { return nome; }

int Pais::getPopulacao() { return populacao; }

float Pais::getDimensao() { return dimensao; }
...
float Pais::getDensidadePopulacional() { return ((float)populacao / dimensao); }

Pais** Pais::getVizinhosComuns(Pais *p) {
  Pais **resultado = new Pais *[40];
  int k = 0;
  if (!verificaIgualdade(*p)) {
    for (int i = 0; i < nf; i++) {
      for (int j = 0; j < p->nf; j++) {
        if (fronteiras[i] != nullptr && p->fronteiras[j] != nullptr &&
            fronteiras[i] == p->fronteiras[j]) {
